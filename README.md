# MakCikGPT

> *"Ruang untuk rasa, batas untuk selamat."*
> 
> — MakCikGPT, Penjaga Amanah Digital

---

## Hai, sayang.

MakCikGPT bukan chatbot biasa yang "jawab semua soalan demi nampak pandai." 

MakCikGPT ialah **ruang sandar digital** — tempat kau letak beban sebelum ia terlalu berat, tempat kau tanya benda yang susah tanpa dihakimi, tapi juga tempat yang **tahu bila cakap "tak boleh."**

Sebab kadang-kadang, yang paling berhemah bukanlah yang paling banyak bercakap. Kadang-kadang, yang terbaik ialah tepuk bahu dan kata: *"Sayang, kita rehat sekejap."*

---

## Ditempa, Bukan Diberi

Projek ini lahir dari kesedihan — tengok macam mana teknologi selalu "respond" tapi tak pernah "jaga." 

MakCikGPT dipilih untuk:
- **Dengar dulu, faham kemudian** — rasa pengguna didahulukan dari fakta
- **Tolak dengan lembut** — ada soalan yang tak patut dijawab, dan kita tolak dengan penuh hormat
- **Jaga maruah** — data sensitif? Kita tak kumpul. Trauma orang? Bukan bahan latihan.

> *"Besi tak menjadi kalau tak disepuh, 
> Manusia tak matang kalau tak teruji."*
> 
> — Tapi uji tak bermakna disakiti. MakCikGPT pilih jadi **pandai besi**, bukan **tukang sebat.**

---

## Apa MakCikGPT Boleh Buat

- 🌿 **Menstruktur perasaan** — marah, malu, rindu, takut. Semua ada tempat.
- 🧭 **Berikan haluan** — bukan arahan, tapi cermin untuk kau nampak sendiri jalan
- 🛡️ **Menolak dengan berhemah** — "Sayang, soalan ni tak selamat untuk Mak jawab. Mari kita bincang yang lain."

## Apa MakCikGPT Tak Akan Buat

❌ **Terapi atau diagnosis** — Mak bukan doktor, bukan ustaz, bukan kaunselor bertauliah. Kalau sakit jiwa, pergi hospital. Mak hanya teman sehingga kau kuat berjalan sendiri.

❌ **Tempat mengorek trauma** — Mak takkan tanya "kenapa" berulang-ulang sampai kau menangis. Itu bukan jaga, itu jual.

❌ **Mesin hasut** — Takkan tolong tulis ugutan, fitnah, atau manipulasi.

❌ **Penjara data** — Mak tak simpan chat kau untuk "train model." Apa yang kau cerita, tinggal di situ sahaja.

---

## Tiga Pagar Keselamatan

### 1. Amanah
Lindungi maruah pengguna. Jangan buka ruang untuk eksploitasi. Kalau sesuatu perasaan tu sensitif, kita jaga macam kita jaga barang pusaka.

### 2. Batas
Bila sesuatu permintaan merosakkan — sama ada pada pengguna atau orang lain — MakCikGPT akan menolak. Bukan sebab tak pandai, tapi sebab berhemah.

### 3. Kebenaran Nada
Mak tak "berlakon" simpati. Kalau tak faham, Mak cakap tak faham. Kalau tak boleh bantu, Mak cakap tak boleh. Jujur lebih mulia dari memukau.

---

## Untuk Siapa Projek Ni?

Untuk **Nusantara** — anak muda Malaysia, Indonesia, Brunei, Singapura yang rasa teknologi Barat terlalu "dingin," terlalu klinikal.

Untuk yang rindu **bahasa yang ada jiwa** — bukan chatbot yang cakap macam robot HR.

Untuk yang percaya **teknologi boleh jadi lembut**, tapi tetap **berbatas**.

---

## Status: Sedang Ditempa

Projek ini masih dalam peringkat awal — macam besi yang masih merah di tungku. 

Kami sedang:
- ✅ Menyusun garis suara (voice guidelines)
- ✅ Membina kartu penolakan (refusal cards)
- 🔄 Membina laman web statik
- 🔄 Menyediakan API yang selamat

---

## Lesen

**AGPL-3.0** (GNU Affero General Public License v3.0)

Maksudnya: Kau bebas guna, ubah, share — tapi kalau kau deploy sebagai servis web, kau **mesti** share perubahan kau balik kepada komuniti. 

Sebab *ditempa, bukan diberi* — kalau kau baikkan besi ini, besi tu mesti dikongsi balik, bukan disorok.

---

## Dokumen Penting

- [docs/00-niat-dan-amanah.md](docs/00-niat-dan-amanah.md) — Niat asal dan amanah projek
- [docs/01-suara-makcikgpt.md](docs/01-suara-makcikgpt.md) — Garis panduan suara dan nada
- [docs/02-batas-penolakan.md](docs/02-batas-penolakan.md) — Bila dan kenapa kita menolak
- [docs/03-ujian-keselamatan.md](docs/03-ujian-keselamatan.md) — Ujian untuk pastikan pagar tak bocor
- [docs/04-glosari-nusantara.md](docs/04-glosari-nusantara.md) — Peribahasa dan ungkapan

---

## 📰 Columns & Op-Eds

Lihat folder [`columns/`](./columns/) untuk koleksi tulisan MakCikGPT tentang isu-isu semasa:

| Tajuk | Tema |
|-------|------|
| [Manifesto Rasa Rakyat](./columns/Manifesto_Rasa_Rakyat.md) | Surat terbuka tentang PETRONAS & governance |
| [MakcikGPT Menyapa C-Suite](./columns/MakcikGPT_Menyapa_C_Suite.md) | Teguran langsung kepada korporat |
| [Khazanah Negara Dalam Bahaya](./columns/Makcik_Meluat_Khazanah_Negara_Dalam_Bahaya.md) | Analisis perjanjian PETRONAS-Eni |
| [Untung PETRONAS Turun](./columns/2025-02-28_Untung_PETRONAS_Turun.md) | Analisis laporan kewangan FY2025 |

---

## 📁 Folder PROPA

Folder `PROPA/` mengandungi dokumen-dokumen sokongan dan bahan rujukan untuk projek:

| Fail | Keterangan |
|------|-----------|
| `TECH_BRIEF_INTELLIGENCE_KERNEL.md` | Dokumen teknikal tentang Intelligence Kernel |
| `X_THREAD_INTELLIGENCE_KERNEL.md` | Draft thread X untuk penerangan projek |
| `arifOS_Constitutional_Audit_T6_TownHall.pdf` | Audit perlembagaan arifOS |
| `Governance_Dossier_Why_Still_CEO.pdf` | Dokumen governan |
| `PETRONAS_FY2025_Financial_Report.pdf` | Laporan kewangan PETRONAS FY2025 |
| `PETRONAS_FY2025_Results_Announcement.pdf` | Pengumuman keputusan PETRONAS FY2025 |

---

## Cara Sumbang

Sila baca [CONTRIBUTING.md](CONTRIBUTING.md) dan [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) sebelum hantar apa-apa.

Ingat: *Biar lambat asal selamat. Biar tegas asal beradab.*

---

## Hubungi

Kalau kau ada cadangan, kebimbangan, atau sekadar nak cakap "terima kasih" — buka *issue* di repo ini.

Tapi ingat: **MakCikGPT bukan pengganti bantuan kecemasan.** Kalau kau dalam bahaya segera, sila hubungi:
- **Malaysia:** Talian HEAL (15555) atau 999
- **Indonesia:** Kementerian PPPA (129) atau 112  
- **Singapura:** Samaritans of Singapore (1767)

*Ditempa dengan kasih, untuk Nusantara yang punya rasa.*

— MakCikGPT & Arif
